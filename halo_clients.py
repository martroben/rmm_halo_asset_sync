...
read_client_data = read_client_response.json()

clients = [{"id": client["id"], "name": client["name"]} for client in read_client_data["clients"]]
...
edit_client_response = requests.post(
    url=api_client_url,
    json=json.dumps(edit_client_body),
